 ...
 * provide functions that enable DPU communicate with outside world and
 * cooperate with other DPUs in scope of pipeline execution. The
 * {@link #sendMessage} method should be used to notify user about more serious
 * events like: changing configuration, the fatal error, important debug
 * information .. The number of the massage emitted by single execution should
 * be reasonable small to preserve readability of the message log. For more
 * intensive logging please use slf4j.
 *
 * @author Petyr
 * @see DPU
 */
public interface DPUContext {
    ...
    /**
     * Return true if the execution of current DPU should be stopped as soon as
